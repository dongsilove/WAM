	...
	$("#detailForm").validate({ // form.submit()을 하면 호출된다. ajaxForm()보다 먼저 호출됨
		...
		submitHandler : function () { //validation이 끝난 이후의 submit 직전 추가 작업할 부분
			console.log("validation 성공 이후 ");
 			/*_ajaxUtils.ajax({"url" : "/api/asstfcltys/", "method": "PUT", "form" : $("#detailForm")
...
var _list = {
	...
	,goEdit(asstSn) {
		...
		window.location = "/asstfclty/edit?asstSn="+asstSn + "&params=" + params;
	}
	,goList() {
	...
	,getDetail : function(asstSn) {
		...
		// 첨부파일조회
		_cmFile.list("T_AM_ASST_FCLTY",asstSn).done(function(list) { 
	...
	,deleteOne : function() {
		...
		if(confirm("삭제하시겠습니까? 삭제 후에는 복구가 불가능 합니다.")) {
			// 삭제
			_ajaxUtils.ajax({"url" : "/api/asstfcltys/"+pk, "method": "DELETE"
